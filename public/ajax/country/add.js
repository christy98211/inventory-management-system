...
function findOneCountry(id) {
    $.ajax({
        url: '/country-list/find-one/'+id,
        type: 'GET',
        headers: {
...
$('#add_country').on('click',function (e) {
    ...
    if (!validate_flag) {
        ...
        $.ajax({
            url: '/country-add/save', // route path
            type: 'POST',
            ...
            'success': function (data, textStatus, request) {
                ...
                if (data.status == 200) {
                    swal.fire({
                        title: "Save",
                        text: "Country Saved Sucessfully",
                        icon: "success",
                        button: true,
                    }).then((value) => {
                        if (value) {
...
$('#edit_country').on('click',function (e) {
    ...
    if (!validate_flag) {
        ...
        $.ajax({
            url: '/country-add/update?'+id, // route path
            type: 'POST',
            data: JSON.stringify(data),
            headers: {
                'Content-Type': "application/json",
            ...
            'success': function (data, textStatus, request) {
                ...
                if (data.status == 200) {                    
                    swal.fire({
                        title: "Update",
                        text: "Country Updated Sucessfully",
                        icon: "success",
                        button: true,
                    }).then((value) => {
                        if (value) {
