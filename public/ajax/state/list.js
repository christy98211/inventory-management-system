...
function getAllStateList(limit, offset, search_data) {
    let searchTerm = search_data ? search_data : ""
    $.ajax({
        url: `/state-list/list-table?limit=${limit}&offset=${offset}&search=${searchTerm}`,
        type: 'GET',
        headers: {
                ...
                $.each(data_rows, function (i, item) {
                    ...
                    <td>
                        <div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                            <button type="button" class="btn btn-primary edit_state" data-id="${item._id}">Edit</button>
                            <button type="button" class="btn btn-primary view_state" data-id="${item._id}">View</button>
                            <button type="button" class="btn btn-primary delete_state" data-id="${item._id}">Delete</button>
                        </div>
                    </td>
                </tr>`;
                });
...
$(document).ready(function () {
    ...
    let limit = 10;
    let search_data = "";      
    getAllStateList(limit, offset, search_data);
});
...
$(document).on('click', '.view_state', function () {
    let id = $(this).attr('data-id');
    window.location.href = `/state-add?id=` + id+'&view=true';
});
...
$(document).on('click', '.edit_state', function () {
    let id = $(this).attr('data-id');
    window.location.href = `/state-add?id=` + id;
});
...
$(document).on('click', '.delete_state', function (e) {
    ...
    $.ajax({
        url: '/state-add/delete?'+id, // route path
        type: 'POST',
        ...
        'success': function (data, textStatus, request) {
            ...
            if (data.status == 200) {                    
                swal.fire({
                    title: "Deleted",
                    text: "State Deactivated Sucessfully",
                    icon: "success",
                    button: true,
                }).then((value) => {
                    if (value) {
