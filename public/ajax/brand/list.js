...
function getAllBrandList(limit, offset, search_data) {
    let searchTerm = search_data ? search_data : ""
    $.ajax({
        url: `/brand-list/list-table?limit=${limit}&offset=${offset}&search=${searchTerm}`,
        type: 'GET',
        headers: {
                ...
                $.each(data_rows, function (i, item) {
                    ...
                    <td>
                        <div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                            <button type="button" class="btn btn-primary edit_brand" data-id="${item._id}">Edit</button>
                            <button type="button" class="btn btn-primary view_brand" data-id="${item._id}">View</button>
                            <button type="button" class="btn btn-primary delete_brand" data-id="${item._id}">Delete</button>
                        </div>
                    </td>
                </tr>`;
                });
...
$(document).ready(function () {
    ...
    let limit = 10;
    let search_data = "";
    getAllBrandList(limit, offset, search_data);
});
...
$(document).on('click', '.view_brand', function () {
    ...
    window.location.href = `/brand-add?id=` + id+'&view=true';
});
...
$(document).on('click', '.edit_brand', function () {
    let id = $(this).attr('data-id');
    window.location.href = `/brand-add?id=` + id;
});
...
$(document).on('click', '.delete_brand', function (e) {
    ...
    $.ajax({
        url: '/brand-add/delete?'+id, // route path
        type: 'POST',
        ...
        'success': function (data, textStatus, request) {
            ...
            if (data.status == 200) {                    
                swal.fire({
                    title: "Deleted",
                    text: "Brand Deactivated Sucessfully",
                    icon: "success",
                    button: true,
                }).then((value) => {
                    if (value) {
