...
function getAllSalesList(limit, offset, search_data) {
    let searchTerm = search_data ? search_data : ""
    $.ajax({
        url: `/sales-list/list-table?limit=${limit}&offset=${offset}&search=${searchTerm}`,
        type: 'GET',
        headers: {
          ...
          if (data.status == 200) {
            ...
            if (data.data[0].rows.length >= 1) {
                ...
                $.each(data_rows, function (i, item) {
                    ...
                    tr += `<tr>
                    <td>${item.customers[0] ? item.customers[0].customer_id : 'NA' }</td>
                    <td>${item.customers[0] ? item.customers[0].customer_name : 'NA' }</td>
                    ...
                    <td>
                        <div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                            <button type="button" class="btn btn-primary edit_sales" data-id="${item._id}">Edit</button>
                            <button type="button" class="btn btn-primary view_sales" data-id="${item._id}">View</button>
                            <button type="button" class="btn btn-primary delete_sales" data-id="${item._id}">Delete</button>
                        </div>
                    </td> 
                </tr>`;
                });
...
$(document).ready(function () {
    ...
    let limit = 10;
    let search_data = "";    
    getAllSalesList(limit, offset, search_data);
});
...
$(document).on('click', '.view_sales', function () {
    ...
    window.location.href = `/sales-add?id=` + id+'&view=true';
});
...
$(document).on('click', '.edit_sales', function () {
    let id = $(this).attr('data-id');
    window.location.href = `/sales-add?id=` + id;
});
...
$(document).on('click', '.delete_sales', function (e) {
    ...
    $.ajax({
        url: '/sales-add/delete?'+id, // route path
        type: 'POST',
        ...
        'success': function (data, textStatus, request) {
            ...
            if (data.status == 200) {                    
                swal.fire({
                    title: "Deleted",
                    text: "Sales Deactivated Sucessfully",
                    icon: "success",
                    button: true,
                }).then((value) => {
                    if (value) {
