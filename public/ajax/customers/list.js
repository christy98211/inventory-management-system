...
function getAllCustomersList(limit, offset, search_data) {
    let searchTerm = search_data ? search_data : ""
    $.ajax({
        url: `/customers-list/list-table?limit=${limit}&offset=${offset}&search=${searchTerm}`,
        type: 'GET',
        headers: {
          ...
          if (data.status == 200) {
            if (data.data[0].rows.length >= 1) {
                ...
                $.each(data_rows, function (i, item) { 
                    ...
                    tr += `<tr>
                    <th><span class="co-name">${item.customer_id}</span></th>
                    <th><span class="co-name">${item.customer_name}</span></th>
                    <td data-priority="1">${item.address}</td>
                    <td data-priority="1">${item.contact}</td>
                    <td data-priority="1">${item.description}</td>
                    <td data-priority="1">${moment(item.created_at).format('lll')}</td>
                    <td data-priority="1">${item.user[0] ? item.user[0].name : 'NA' }</td>
                    <td data-priority="1">${item.is_active}</td>
                    <td>
                        <div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                            <button type="button" class="btn btn-primary edit_customers" data-id="${item._id}">Edit</button>
                            <button type="button" class="btn btn-primary view_customers" data-id="${item._id}">View</button>
                            <button type="button" class="btn btn-primary delete_customers" data-id="${item._id}">Delete</button>
                        </div>
                    </td>
                </tr>`; 
                });
...
$(document).ready(function () {
    ...
    let limit = 10;
    let search_data = "";      
    getAllCustomersList(limit, offset, search_data);
});
...
$(document).on('click', '.view_customers', function () {
    let id = $(this).attr('data-id');
    window.location.href = `/customers-add?id=` + id+'&view=true';
});
...
$('#search_data').keyup(function () {
    ...
    if (page_number_input <= pages_left) {
        ...
        offset = page_number_dropdown * (page_number_input - 1);
        let limit = page_number_dropdown;
        getAllRejectionList(limit, offset, search_data);
    }
});


// on click of edit button
$(document).on('click', '.edit_customers', function () {
    let id = $(this).attr('data-id');
    window.location.href = `/customers-add?id=` + id;
});
...
$(document).on('click', '.delete_customers', function (e) {
    ...
    $.ajax({
        url: '/customers-add/delete?'+id, // route path
        type: 'POST',
        ...
        'success': function (data, textStatus, request) {
            ...
            if (data.status == 200) {                    
                swal.fire({
                    title: "Deleted",
                    text: "Customer Deactivated Sucessfully",
                    icon: "success",
                    button: true,
                }).then((value) => {
                    if (value) {
