...
function getAllLocationList(limit, offset, search_data) {
    let searchTerm = search_data ? search_data : ""
    $.ajax({
        url: `/location-list/list-table?limit=${limit}&offset=${offset}&search=${searchTerm}`,
        type: 'GET',
        headers: {
          ...
          if (data.status == 200) {
            ...
            if (data.data[0].rows.length >= 1) {
                ...
                $.each(data_rows, function (i, item) {
                    ...
                    tr += `<tr>
                    <th><span class="co-name">${item.city}</span></th>
                    <td>${item.state[0] ? item.state[0].name : 'NA' }</td>
                    <td>${item.country[0] ? item.country[0].name : 'NA' }</td>
                    <td>${moment(item.created_at).format('lll')}</td>
                    <td>${item.user[0] ? item.user[0].name : 'NA' }</td>
                    <td>${item.is_active}</td>
                    <td>
                        <div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                            <button type="button" class="btn btn-primary edit_location" data-id="${item._id}">Edit</button>
                            <button type="button" class="btn btn-primary view_location" data-id="${item._id}">View</button>
                            <button type="button" class="btn btn-primary delete_location" data-id="${item._id}">Delete</button>
                        </div>
                    </td>
                </tr>`;
                });
...
$(document).ready(function () {
    ...
    let limit = 10;
    let search_data = "";    
    getAllLocationList(limit, offset, search_data);
});
...
$(document).on('click', '.view_location', function () {
    ...
    window.location.href = `/location-add?id=` + id+'&view=true';
});
...
$(document).on('click', '.edit_location', function () {
    let id = $(this).attr('data-id');
    window.location.href = `/location-add?id=` + id;
});
...
$(document).on('click', '.delete_location', function (e) {
    ...
    $.ajax({
        url: '/location-add/delete?'+id, // route path
        type: 'POST',
        ...
        'success': function (data, textStatus, request) {
            ...
            if (data.status == 200) {                    
                swal.fire({
                    title: "Deleted",
                    text: "Location Deactivated Sucessfully",
                    icon: "success",
                    button: true,
                }).then((value) => {
                    if (value) {
